...
public class DetailActivity extends AppCompatActivity {
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    descriptionTextView = findViewById(id.detail_description);
    dueDateTimeTextView = findViewById(id.detail_dueDateTime);
    completionDateTimeTextView = findViewById(id.detail_doneDateTime);
    favouriteIconImageView = findViewById(id.detail_favourite_icon);
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    switch (item.getItemId()) {
      ...
      case id.action_delete_todo:
        new Builder(this).setTitle("Delete todo")
          .setMessage("Do you really want to delete this todo?")
          .setIcon(R.drawable.ic_dialog_alert)
          .setPositiveButton(string.yes, (dialog, whichButton) -> deleteTodo())
  ...
  public void removeContact(final View clickedView) {
    ofNullable((TextView) ((View) clickedView.getParent()).findViewById(id.contact_id)).ifPresent((TextView view) -> {
      final String contactId = view.getText().toString();
      if (!contactId.isEmpty()) {
        contactRepository.deleteById(contactId);
        final Set<String> newContactIds = todo.getContactIds();
        newContactIds.remove(contactId);
        final Todo updatedTodo = todo.toBuilder().contactIds(newContactIds).build();
  ...
  private void updateUI() {
    ...
    favouriteIconImageView.setImageResource(todo.isFavorite() ? drawable.ic_star_black_24dp : drawable.ic_star_border_black_24dp);
